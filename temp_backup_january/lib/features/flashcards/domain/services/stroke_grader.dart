...
class StrokeGrader {
  ...
  static double _gradeSingleStroke(Path refPath, List<Offset> userStroke) {
    ...
    // Threshold is ~150 units in 1000x1000 space
    double startScore = (1.0 - (userStart - refStart).distance / 250.0).clamp(0.0, 1.0);
    double endScore = (1.0 - (userEnd - refEnd).distance / 250.0).clamp(0.0, 1.0);
    ...
    for (final p in userStroke) {
      ...
      // If within 100 units (10% of canvas), consider it 'on the line'
      if (minStrokeDist < 100.0) {
        onPathCount++;
      }
