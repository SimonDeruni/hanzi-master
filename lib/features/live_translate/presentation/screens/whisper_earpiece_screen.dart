...
class WhisperMessage {
  ...
  WhisperMessage({required this.originalText, required this.translatedText});

  WhisperMessage copyWith({String? originalText, String? translatedText}) {
...
class WhisperEarpieceScreen extends ConsumerStatefulWidget {
  const WhisperEarpieceScreen({super.key});

  @override
  ConsumerState<WhisperEarpieceScreen> createState() => _WhisperEarpieceScreenState();
}

class _WhisperEarpieceScreenState extends ConsumerState<WhisperEarpieceScreen> {
  ...
  final List<WhisperMessage> _transcript = [];
  ...
  void _handleTranslatedText(String text) {
    ...
    setState(() {
      if (_transcript.isNotEmpty) {
        ...
        _transcript[_transcript.length - 1] = last.copyWith(translatedText: last.translatedText + text);
      } else {
        _transcript.add(WhisperMessage(originalText: "...", translatedText: text));
      }
    });
  }
  ...
  void _handleOriginalAudioText(String text, bool finished) {
    ...
    setState(() {
      if (_transcript.isEmpty || finished) {
        _transcript.add(WhisperMessage(originalText: text, translatedText: ""));
      } else {
        final last = _transcript.last;
        _transcript[_transcript.length - 1] = last.copyWith(originalText: text);
      }
    });
  }
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: isDark ? Colors.purple.withValues(alpha: 0.1) : Colors.purple.withValues(alpha: 0.05),
                      ...
                      _buildInstructionRow(
                        Icons.headphones,
                        "Wear Earbuds",
                        "Best experienced with headphones for clear translations.",
                        isDark,
                      ),
                      ...
                      _buildInstructionRow(
                        Icons.volume_up,
                        "Listen to Mandarin",
                        "Point your phone towards the Chinese speaker.",
                        isDark,
                      ),
                      const SizedBox(height: 24),
                      _buildInstructionRow(
    ...
    return Row(
      ...
      children: [
        Container(
          ...
          child: Icon(icon, color: isDark ? Colors.purple.shade300 : Colors.purple.shade600, size: 24),
        ),
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: _isLive ? Colors.purple.withValues(alpha: 0.1) : Colors.grey.withValues(alpha: 0.1),
                  borderRadius: BorderRadius.circular(24),
                  border: Border.all(color: _isLive ? Colors.purple.shade200 : Colors.grey.shade400),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(
                      _isLive ? Icons.earbuds : Icons.earbuds_battery,
                      color: _isLive ? Colors.purple.shade700 : Colors.grey,
                    ),
                    const SizedBox(width: 8),
                    Text(
                      _status,
                      style: TextStyle(
                        color: _isLive ? Colors.purple.shade900 : Colors.grey.shade600,
                        fontWeight: FontWeight.bold,
                      ),
                    ),
                  ],
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Icon(Icons.hearing, size: 16, color: isDark ? Colors.white54 : Colors.black54),
                              const SizedBox(width: 8),
                              Expanded(
                                child: Text(
                                  msg.originalText,
